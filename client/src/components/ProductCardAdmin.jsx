...
const ProductCardAdmin = ({ data, fetchProductData }) => {
  ...
  return (
    <div className="w-39 p-4 bg-white rounded border border-green-600">
      <div>
        <img
          src={data?.image[0]}
          alt={data?.name}
      ...
      <div className="grid grid-cols-2 gap-3 py-2 font-medium">
        <button
          onClick={() => setEditOpen(true)}
          className="border px-1 py-1 text-sm border-green-600 bg-green-100 text-green-800 hover:bg-green-200 rounded-full"
        >
          Edit
        </button>
        <button
          onClick={() => setOpenDelete(true)}
          className="border px-1 py-1 text-sm border-red-600 bg-red-100 text-red-600 hover:bg-red-200 rounded-full"
        >
          Delete
        </button>
      </div>
